...
function DoOpenAndVerifyGame(appName, game_foler_name)
    ...
    if DoesDirectoryExist(appPath) then
        ...
        if DoesDirectoryExist(gamePath) then
            ...
            hs.notify
                .new({ title = "Game folder: '" .. game_foler_name .. "' not found", informativeText = "run it failed" })
                :send()
        end
    else
...
function DoPlayGame(appName, runApp)
    hs.notify.new({ title = "Playing the game, " .. appName, informativeText = "Start it" }):send()
